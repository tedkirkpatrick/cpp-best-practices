...
/*
predicate pureOpenDefinition(StackVariable v, ControlFlowNode def) {
  exists(Expr expr | exprDefinition(v, def, expr))
}
*/
/*
from StackVariable v
   , ControlFlowNode def
   //, ReturnStmt ret
where
  //openDefinition(v, def) and
  pureOpenDefinition(v, def) and
  ...
  //v.getName() = "sockfd"
  v.hasQualifiedName("", "", "logger")
select //ret,
  def,
  "Descriptor assigned to '" + v.getName().toString() + "' (line " +
    def.getLocation().getStartLine().toString() + ") found."
*/
...
select
  v, "Found " + v.getName().toString() + ", in " + def.toString() + " reaching " + ret.toString() + ": " + ret.getExpr().toString() + " checked success not getAChild " + " other exists "
